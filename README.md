...
> _A voice journal that knows when you need people and finds exactly the right ones._
...
2. **Persona engine** : your _stage_ (In the storm / Finding ground / Through it) is derived from your recent entries, not stored as a label. A user who joins already calm can reach _Through it_ without ever touching _In the storm_. The system is a GPS, not a subway line.
...
| Voice transcription      | Groq Whisper API _(optional)_                    |
| Acoustic features        | wav2vec2-base (HuggingFace Transformers)         |
| LLM : persona extraction | Gemini 2.5 Flash                                 |
| LLM : memory wall        | Gemini 1.5 Flash _(configurable)_                |
| Clustering               | HDBSCAN                                          |
...
- A [Groq](https://console.groq.com) API key _(optional : only needed for voice transcription)_
...
- Circle conversations are anonymised before any LLM call, user IDs are replaced with stage role labels (_In the storm_, _Finding ground_, _Through it_). No raw transcripts are stored permanently.
